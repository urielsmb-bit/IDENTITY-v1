...
export type ControlId =
  ...
  // caja
  | 'superficie'
  /* Los de la caja que el bloque YA TRAE —musica y Discord—, que no es la
     misma cosa que `superficie`: aquella AÑADE una caja, y estos ajustan
     la que ya hay. Ponerle una encima dibujaba dos, una dentro de otra. */
  | 'cajaPropia'
  | 'colorCaja'
  | 'opacidadCaja'
  | 'colorBorde'
...
/**
 * La caja de un bloque que YA TIENE caja.
 *
 * Musica y Discord se pintan dentro de su propio recuadro: fondo, borde y
 * radio de fabrica. Con el grupo `CAJA` normal se les añadia OTRA encima y
 * quedaba una caja dentro de otra — el borde de la de dentro cruzando el
 * relleno de la de fuera. Y no habia forma de arreglarlo desde el editor,
 * porque lo unico ajustable era la de fuera.
 *
 * Asi que aqui no esta `superficie` —la que añade— ni `heredarCaja`. Lo que
 * hay son los mandos de la que ya tiene, incluido apagarla del todo.
 */
const CAJA_PROPIA: GrupoControles = {
  titulo: 'Caja',
  controles: [
    'cajaPropia',
    'colorCaja', 'opacidadCaja',
    'colorBorde', 'grosorBorde', 'borde',
    'radio', 'relleno', 'ancho', 'desenfoque', 'brillo',
  ],
};
