-- ============================================================
-- IDENTITY · APLICAR 0018 · el estado de Discord sin Lanyard
--
-- Pegar entero en:  supabase.com/dashboard -> tu proyecto -> SQL Editor
--
-- POR QUE A MANO Y NO CON `db push`: la tabla que lleva la cuenta de las
-- migraciones esta vacia —las primeras se aplicaron pegando— asi que
-- `db push` intentaria lanzar las dieciocho desde cero contra una base
-- con gente dentro. Lo dice DESPLIEGUE.md y sigue siendo verdad.
--
-- Se puede relanzar sin romper nada: todo va con `if not exists` y las
-- politicas se tiran antes de crearse.
-- ============================================================
...
-- Deberia devolver una fila: la tabla, con RLS encendida.
select
  c.relname                as tabla,
  c.relrowsecurity         as rls_encendida,
  (select count(*) from pg_policies
     where schemaname = 'public' and tablename = 'presencia') as politicas
from pg_class c
join pg_namespace n on n.oid = c.relnamespace
where n.nspname = 'public' and c.relname = 'presencia';
